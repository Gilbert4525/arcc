-- COMPLETE VOTING EMAIL FIX - CORRECTED VERSION
-- This script will fix the voting completion detection and email triggering
-- Uses correct column names: voter_id (not user_id)
...
BEGIN
    ...
    IF item_type = 'resolution' THEN
        ...
        SELECT status, voting_deadline INTO item_status, item_deadline
        FROM resolutions 
        WHERE id = item_id;
        
        -- Count votes for this resolution (using correct column name: voter_id)
    ...
    ELSIF item_type = 'minutes' THEN
        ...
        -- Count votes for this minutes (using correct column name: user_id)
        SELECT COUNT(*) INTO total_votes
        FROM minutes_votes 
    ...
    IF completion_reason = 'not_complete' THEN
        ...
        ELSIF item_deadline IS NOT NULL AND item_deadline <= NOW() THEN
            is_complete := true;
    ...
    result := jsonb_build_object(
        ...
        'deadlineExpired', CASE WHEN item_deadline IS NOT NULL THEN item_deadline <= NOW() ELSE false END,
        'itemStatus', item_status,
        'votingDeadline', item_deadline
    );
