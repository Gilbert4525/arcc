...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Error fetching minutes documents:', error);
      return NextResponse.json({ error: 'Failed to fetch minutes documents' }, { status: 500 });
    }
    ...
    let countQuery = supabase
      ...
      .or('tags.cs.{"minutes"},tags.cs.{"meeting_minutes"}');
