...
interface MinutesDocumentUploadProps {
  categories: Category[];
}
...
export function MinutesDocumentUpload({ categories }: MinutesDocumentUploadProps) {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    const newFiles = acceptedFiles.map(file => ({
      ...
      description: '',
      categoryId: '',
      tags: ['minutes', 'meeting_minutes'], // Default tags for minutes documents
      progress: 0,
  ...
  const handleUploadAll = async () => {
    ...
    try {
      ...
      toast.success('All minutes documents uploaded successfully!');
  ...
  const minutesCategories = categories.filter(cat => 
    cat.type === 'meeting' || cat.type === 'document'
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Upload Minutes Documents</h1>
          <p className="text-gray-600">Upload meeting minutes and related documents</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Upload Minutes Documents</CardTitle>
        </CardHeader>
        <CardContent>
          <div
            {...getRootProps()}
          ...
          >
            ...
            {isDragActive ? (
              <p className="text-lg">Drop the minutes documents here ...</p>
            ) : (
              <div>
                <p className="text-lg mb-2">
                  Drag & drop minutes documents here, or click to select files
                </p>
      ...
      {uploadFiles.length > 0 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>Minutes Documents to Upload ({uploadFiles.length})</CardTitle>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            placeholder="Minutes document title"
                            required
                          />
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            ...
                            <SelectContent>
                              {minutesCategories.map(category => (
                        ...
                        <div className="md:col-span-2 space-y-2">
                          ...
                          <Textarea
                            ...
                            placeholder="Describe this minutes document (optional)"
                            rows={3}
                          />
                        </div>
                        ...
                        <div className="md:col-span-2 space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2 mb-2">
                            {uploadFile.tags.map((tag, index) => (
                              <Badge key={index} variant="secondary" className="cursor-pointer">
                                {tag}
                                {!['minutes', 'meeting_minutes'].includes(tag) && (
                                  <X 
                          ...
                          <p className="text-xs text-muted-foreground">
                            Default tags: minutes, meeting_minutes
                          </p>
                        </div>
                      </div>
                    )}
