...
interface PassedResolutionDocumentUploadProps {
  categories: Category[];
}
...
export function PassedResolutionDocumentUpload({ categories }: PassedResolutionDocumentUploadProps) {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    const newFiles = acceptedFiles.map(file => ({
      ...
      description: '',
      categoryId: '',
      tags: ['passed_resolution', 'resolution'], // Default tags for resolution documents
      progress: 0,
  ...
  const handleUploadAll = async () => {
    ...
    try {
      ...
      toast.success('All resolution documents uploaded successfully!');
  ...
  const addTag = (fileId: string, tag: string) => {
    ...
    if (!file.tags.includes(tag.trim())) {
      updateFileField(fileId, 'tags', [...file.tags, tag.trim()]);
    }
  };
  ...
  // Filter categories to show only resolution and document types
  const resolutionCategories = categories.filter(cat => 
    cat.type === 'resolution' || cat.type === 'document'
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Upload Passed Resolution Documents</h1>
          <p className="text-gray-600">Upload documents for resolutions that have been passed by the board</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Upload Resolution Documents</CardTitle>
        </CardHeader>
        <CardContent>
          <div
            {...getRootProps()}
          ...
          >
            ...
            {isDragActive ? (
              <p className="text-lg">Drop the resolution documents here ...</p>
            ) : (
              <div>
                <p className="text-lg mb-2">
                  Drag & drop resolution documents here, or click to select files
                </p>
      ...
      {uploadFiles.length > 0 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>Resolution Documents to Upload ({uploadFiles.length})</CardTitle>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            placeholder="Resolution document title"
                            required
                          />
                        </div>
                        ...
                        <div className="md:col-span-2 space-y-2">
                          ...
                          <Textarea
                            ...
                            placeholder="Describe this resolution document (optional)"
                            rows={3}
                          />
                        </div>
                        ...
                        <div className="md:col-span-2 space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2 mb-2">
                            {uploadFile.tags.map((tag, index) => (
                              <Badge key={index} variant="secondary" className="cursor-pointer">
                                {tag}
                                {/* Don't allow removal of default resolution tags */}
                                {!['passed_resolution', 'resolution'].includes(tag) && (
                                  <X 
                          ...
                          <Input
                            placeholder="Add additional tag and press Enter"
                            onKeyPress={(e) => {
                              if (e.key === 'Enter') {
                          ...
                          <p className="text-xs text-muted-foreground">
                            Default tags: passed_resolution, resolution
                          </p>
                        </div>
                      </div>
                    )}
