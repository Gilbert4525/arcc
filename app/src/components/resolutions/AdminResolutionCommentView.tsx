...
export function AdminResolutionCommentView({ open, onOpenChange, resolution }: AdminResolutionCommentViewProps) {
  ...
  useEffect(() => {
    if (open && resolution.id) {
      fetchComments();
    }
  }, [open, resolution.id, showOnlyComments]);

  const fetchComments = async (retryCount = 0) => {
    try {
      setLoading(true);

      console.log('AdminResolutionCommentView - Fetching votes for resolution:', resolution.id);
      ...
      if (error) {
        if (error.code && error.code.includes('50') && retryCount < 2) {
      ...
      const stats: CommentStatistics = {
        ...
        total_comments: allVotes.filter(vote => vote.vote_reason && vote.vote_reason.trim()).length,
        comments_by_vote: {
          for: allVotes.filter(vote => vote.vote === 'for' && vote.vote_reason && vote.vote_reason.trim()).length,
          against: allVotes.filter(vote => vote.vote === 'against' && vote.vote_reason && vote.vote_reason.trim()).length,
          abstain: allVotes.filter(vote => vote.vote === 'abstain' && vote.vote_reason && vote.vote_reason.trim()).length,
        },
        avg_comment_length: 0,
        has_concerns: allVotes.some(vote => vote.vote === 'against' && vote.vote_reason && vote.vote_reason.trim())
      };

      // Calculate average comment length
      const commentsWithText = allVotes.filter(vote => vote.vote_reason && vote.vote_reason.trim());
      if (commentsWithText.length > 0) {
        stats.avg_comment_length = commentsWithText.reduce((sum, vote) => 
          sum + (vote.vote_reason?.length || 0), 0) / commentsWithText.length;
      }

      setStatistics(stats);
      console.log('AdminResolutionCommentView - Statistics:', stats);
  ...
  const VoteCard = ({ vote }: { vote: VoteWithProfile }) => (
    <Card key={vote.id} className="mb-3">
      ...
      {vote.vote_reason && vote.vote_reason.trim() && (
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            Voting Comments - {resolution.title}
          </DialogTitle>
          <DialogDescription className="flex items-center gap-4">
            ...
            <span className="flex items-center gap-1">
              ...
              Created: {formatDate(resolution.created_at)}
            </span>
          </DialogDescription>
        </DialogHeader>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <div className="text-center p-3 bg-orange-50 rounded-lg">
                      <div className="text-2xl font-bold text-orange-600">{statistics.comments_by_vote.against}</div>
                      <div className="text-sm text-gray-600">Against Comments</div>
                    </div>
