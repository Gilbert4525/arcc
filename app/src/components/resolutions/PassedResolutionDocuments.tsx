...
interface PassedResolutionDocumentsProps {
...
export function PassedResolutionDocuments({
  initialDocuments,
  categories,
  userId,
  userRole
}: PassedResolutionDocumentsProps) {
  ...
  useDocumentsRealtime((updatedDocument: any) => {
    if (updatedDocument && updatedDocument.id) {
      // Only handle resolution documents (filter by tags or category)
      const isResolutionDocument = updatedDocument.tags?.includes('passed_resolution') || 
        updatedDocument.tags?.includes('resolution');
      
      if (!isResolutionDocument) return;

      // Handle document deletion
      if (updatedDocument._deleted) {
        setDocuments(prev => prev.filter(doc => doc.id !== updatedDocument.id));
        toast.info(`Resolution document "${updatedDocument.title}" was deleted`);
        return;
      }
      ...
      if (updatedDocument.created_by !== userId) {
        toast.info(`New resolution document uploaded: ${updatedDocument.title}`);
      }
    }
  });
  ...
  const handleDownload = async (doc: Document) => {
    try {
      ...
      toast.success('Resolution document downloaded successfully');
    } catch (error) {
      console.error('Download error:', error);
      toast.error('Failed to download resolution document');
    }
  };
  ...
  const handlePreview = async (document: Document) => {
    try {
      ...
      toast.success('Resolution document opened for viewing');
    } catch (error) {
      console.error('Preview error:', error);
      toast.error('Failed to preview resolution document');
    }
  };
  ...
  const handleTogglePublished = async (documentId: string, isPublished: boolean) => {
    try {
      ...
      if (error) {
        toast.error(`Failed to ${isPublished ? 'publish' : 'unpublish'} resolution document`);
        return;
      }
      ...
      toast.success(`Resolution document ${isPublished ? 'published' : 'unpublished'} successfully`);
    } catch {
  ...
  const handleEditDocument = async (formData: FormData) => {
    ...
    try {
      ...
      if (error) {
        toast.error(`Failed to update resolution document: ${error.message}`);
        return;
      }
      ...
      toast.success('Resolution document updated successfully');
    } catch {
      toast.error('An unexpected error occurred');
    } finally {
      setLoading(false);
  ...
  const handleDeleteDocument = async (documentId: string, filePath: string) => {
    try {
      ...
      if (dbError) {
        toast.error(`Failed to delete resolution document: ${dbError.message}`);
        return;
      }

      setDocuments(documents.filter(doc => doc.id !== documentId));
      toast.success('Resolution document deleted successfully');
    } catch {
      toast.error('An unexpected error occurred');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Passed Resolution Documents</h1>
          <p className="text-gray-600">Manage and access passed resolution documents</p>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center">
        <div className="flex space-x-4">
          <div className="flex-1 max-w-sm relative">
            ...
            <Input
              placeholder="Search resolution documents..."
              value={searchTerm}
          ...
          <Select value={categoryFilter} onValueChange={setCategoryFilter}>
            ...
            <SelectContent>
              ...
              {categories.filter(cat => cat.type === 'resolution' || cat.type === 'document').map(category => (
        ...
        <div className="flex space-x-2">
          ...
          {userRole === 'admin' && (
            <Button asChild>
              <a href="/dashboard/resolutions/documents/upload">
      ...
      {viewMode === 'table' ? (
        <Card>
          <CardHeader>
            <CardTitle>Resolution Documents ({filteredDocuments.length})</CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                          ...
                          {canEditDocument(document) && (
                            <>
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    <AlertDialogTitle>Delete Resolution Document</AlertDialogTitle>
                                    <AlertDialogDescription>
      ...
      {editingDocument && (
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Edit Resolution Document</DialogTitle>
              <DialogDescription>
                Update resolution document information and metadata.
              </DialogDescription>
            </DialogHeader>
            ...
            <form action={handleEditDocument} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="passed_resolution, resolution, board_decision"
                />
              </div>
